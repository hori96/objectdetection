...
class CustomObjectDetection:
    ...
    def detectObjectsFromImage(self, input_image="", output_image_path="", input_type="file", output_type="file",
                               ...
                               display_percentage_probability=True, display_object_name=True, thread_safe=False, rectangle_width=20, text_size=5):
        ...
        if self.__model is None:
            ...
            if self.__model_type == "yolov3":
                ...
                #self.__detection_utils.no_box_overlap(all_boxes, all_labels, all_scores)
                drawn_image = self.__detection_utils.draw_boxes_and_caption(image_frame.copy(), all_boxes, all_labels,
                                                                            ...
                                                                            rect_width=rectangle_width, name_size=text_size)
...
class CustomDetectionUtils:
    ...
    def no_box_overlap(self, v_boxes, v_labels, v_scores):
        print (v_boxes)
        boxlength = 0
        i = 0
        k = 0
        ...
        while i < boxlength:
            ...
            while k < boxlength:
                if k != i:
                    newbox = v_boxes[k]
                    if newbox.ymin < box.ymin and newbox.ymax > box.ymin or newbox.xmin < box.xmin and newbox.xmax > box.xmin:
                        if v_scores[i] > v_scores[k]:
                            v_boxes.pop(i)
                            v_labels.pop(i)
                            v_scores.pop(i)
                            ...
                            v_scores.pop(k)
                            boxlength = boxlength - 1
                    elif newbox.ymin > box.ymin and newbox.ymax < box.ymin or newbox.xmin > box.xmin and newbox.xmax < box.xmin:
                        if v_scores[i] > v_scores[k]:
                            v_boxes.pop(i)
                            v_labels.pop(i)
                            v_scores.pop(i)
    ...
    def draw_boxes_and_caption(self, image_frame, v_boxes, v_labels, v_scores, show_names=False, show_percentage=False, rect_width=20, name_size=5):
	    ...
	    #utolsó paraméter
            image_frame = cv2.rectangle(image_frame, (x1, y1), (x2, y2), class_color, rect_width)

            label = ""
            if show_names and show_percentage:
                label = "%s : %d" % (v_labels[i], v_scores[i])
            elif show_names:
                label = "%s" % (v_labels[i])
		...
		#utolsó paraméter vonalvastagság, 5paraméter betűméret közötte szín
                cv2.putText(image_frame, label, (b[0], b[1] - 10), cv2.FONT_HERSHEY_PLAIN, name_size, (0, 0, 0), 5)
